...
<div class="already account right"><span>Don't have an account?</span><?php echo $this->Html->link('Sign Up', ($this->params['controller'] == 'orders') ? '/checkout/signup' : '/signup', array('class' => 'btn gradient green')); ?>
</div>
<?php elseif ($this->params['action'] == 'sign_up'): ?>
<!-- User Signup -->
<div class="already account right"><span>Already have an account?</span><?php echo $this->Html->link('Login', ($this->params['controller'] == 'orders') ? '/checkout/login' : '/login', array('class' => 'btn gradient green')); ?>
</div>
